...
**Key design points:**
- M2, M3, M4 run in parallel on the **validated** BAM. `FILTER_CHROMS` + `VALIDATE_REF_BAM` are lifted to `svcaller.nf` (top-level, run once, storeDir-cached) so SV, CNV, and SMN all see the same canonical-filtered BAM, not the raw full-hg38 one (whose alt contigs bias CNVpytor's genome-wide depth baseline). FASTQ-derived BAMs are already canonical and skip the filter. **M8 alpha-globin deliberately stays on the RAW `ch_bam`** — its GIAB depth baselines (`validation/giab_alpha_baseline.tsv`) were calibrated on the raw BAM, so moving it needs those baselines re-derived on filtered GIAB BAMs first. M3b (CNV_TRAITS) consumes the validated BAM plus `CNV_CALLING.out.cnv_bed`, so it runs after M3.
...
- **Meta-map consistency for exact joins**: REPORT joins ~18 per-sample channels. A `.join()` keys on the WHOLE `meta` map, so any channel whose meta differs by even one key used to be silently dropped (no report), caught only by the end count-guard which hard-fails the run. `needs_chr_filter` (preprocess.nf) was the recurring landmine: `PICARD_MARKDUP.out.metrics` is produced before the tag, so every FASTQ-derived sample's report vanished until it was re-tagged. **Fixed 2026-07-23:** `report.nf` now normalizes every per-sample meta to a minimal `[id: meta.id]` at the top of `main:` (the `byId` helper), so the joins depend only on the sample id and cannot diverge. Nothing in REPORT/Circos/Truvari reads any meta field but `id`, so this loses nothing; guarded by `tests/test_report_meta_join.py`. The count-guard stays as a backstop. When adding a per-sample channel to REPORT, add it to the `byId(...)` block too.
...
invalidates the storeDir cache, re-costing an hour per sample. **Pre-flight:** `VALIDATE_REF_BAM` (top-level in `svcaller.nf`, right after FILTER_CHROMS) fails fast if the reference has contigs the BAM lacks — catches the canonical-vs-full hg38 mismatch in seconds instead of a silent Manta crash ~4 h in.
