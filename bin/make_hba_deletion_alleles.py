...
SEGMENTS = [
    ("HBZ",        "HBZ",  0,      0,      "zeta-globin; spared by --SEA/--MED, lost in --FIL/--THAI"),
    ("INTER_Z_A",  None,   None,   None,   "between HBZ and HBA2; spans the psi genes"),
    ("HBA2",       "HBA2", 0,      0,      "alpha-2"),
    ("INTER_A2_A1", None,  None,   None,   "between HBA2 and HBA1; deleted in -a3.7"),
    ("HBA1",       "HBA1", 0,      0,      "alpha-1"),
]
...
def build_segments(models):
    ...
    rows = []
    for name, gene, _, _, note in SEGMENTS:
        if gene:
            s, e = g[gene]
        elif name == "INTER_Z_A":
        ...
        rows.append((z.chrom, s, e, name, note))
...
def main():
    ...
    with open(bed, "w") as fh:
        ...
        fh.write("# Normalize against the CTRL_* windows in assets/cnv_trait_regions.bed.\n")
        for chrom, s, e, name, note in segs:
            fh.write(f"{chrom}\t{s-1}\t{e}\t{name}\t# {note}\n")
    ...
    for chrom, s, e, name, _ in segs:
        print(f"  {name:<12} {chrom}:{s}-{e}  ({e-s+1:,} bp)")
