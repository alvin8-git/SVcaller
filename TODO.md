...
### Pending
- [ ] **Audit remaining `|| true` in `bin/*.py` and `validation/*.sh`** — this pass covered `modules/`, `subworkflows/`, `workflows/` and `main.nf`. The Python report generators in `bin/` were not audited for the analogous pattern (treating a missing/empty input as "no findings" rather than an error). OmniGen was hardened on its side, but SVcaller's own `bin/html_report.py` / `bin/smn_report.py` should be checked for the same `os.path.exists()`-style gating.
